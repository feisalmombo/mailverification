    ...
    public function register(Request $request) {
        ...
        if($validator->passes()){
            ...
            Mail::send('mail.activation', $user, function($message) use ($user){
                ...
                $message->subject('umojaswitch.co.tz - Activation Code');
            });

            return redirect()->to('login')->with('message', "We sent activation code, please check your email");
        }
    ...
    public function userActivation($token) {
        ...
        if(!is_null($check)) {
            ...
            if($user->is_activated == 1){
                return redirect()->to('login')->with('message', "User are already activated");
            }
